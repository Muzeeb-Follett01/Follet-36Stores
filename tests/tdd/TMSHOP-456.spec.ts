...
// ── Test Suite ────────────────────────────────────────────────────────────────
test.describe("TMSHOP-456: E2E003 | FMS Standalone - PDP Product Browse/Search", () => {
  test.skip(!isEnabled("TMSHOP-456"), "Skipped via Excel execute flag");
   ...
   */
  test("E2E003 | FMS Standalone - PDP Product Browse/Search", async ({
    page,
  }, testInfo) => {
    const loginPage = new FollettLoginPage(page, "TMSHOP-456");
    const productPage = new FollettProductPage(page, "TMSHOP-456");
    const checkoutPage = new FollettCheckoutPage(page, "TMSHOP-456");
