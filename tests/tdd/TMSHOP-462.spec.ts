...
// ── Test Suite ────────────────────────────────────────────────────────────────
test.describe("TMSHOP-462: FMS Standalone - Course Material Browse", () => {
  test.skip(!isEnabled("TMSHOP-462"), "Skipped via Excel execute flag");

  /**
   * End-to-end test (TMSHOP-462):
   ...
   *  N+5. Complete Order
   */
  test("FMS Standalone - Course Material Browse", async ({
    page,
  }, testInfo) => {
    const loginPage = new FollettLoginPage(page, "TMSHOP-462");
    const courseBuilder = new FollettCourseBuilderPage(page, "TMSHOP-462");
    const productPage = new FollettProductPage(page, "TMSHOP-462");
    const checkoutPage = new FollettCheckoutPage(page, "TMSHOP-462");
    ...
    await checkoutPage.fillRentalAgreements(testInfo);
