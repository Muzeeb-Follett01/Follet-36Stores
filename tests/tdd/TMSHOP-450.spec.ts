...
const products = td.products ?? []; // search-based products
const courses = td.courses ?? []; // course entries for textbook flow
const courseProducts = td.courseProducts ?? []; // textbook products (paired with courses)
const categoryProducts = td.categoryProducts ?? []; // Shop-By category products
const delivery = td.delivery;
const payment = td.payment;
const studentId = td.studentId;
const signedInUser = td.signedInUser;
const payNowOption = td.payNowOption;

// ── Test Suite ────────────────────────────────────────────────────────────────
test.describe("TMSHOP-450: E2E001 | FMS Standalone - Basic Flow, Course Material", () => {
  test.skip(!isEnabled("TMSHOP-450"), "Skipped via Excel execute flag");

  test("E2E001 | FMS Standalone - Basic Flow, Course Material", async ({
    page,
  }, testInfo) => {
    const loginPage = new FollettLoginPage(page, "TMSHOP-450");
    const courseBuilder = new FollettCourseBuilderPage(page, "TMSHOP-450");
    const productPage = new FollettProductPage(page, "TMSHOP-450");
    const checkoutPage = new FollettCheckoutPage(page, "TMSHOP-450");
